...
int main(int argc, char *argv[]) {
    ...
    if (db){
        ...
        for (int i = 0; i < size; ++i){ // process all the student records in database
            ...
            char name_uniq = 1, surname_uniq = 1;

            for(int j = 0; j < size; j++){
                if((i != j) && (!strcmp(students[i].name, students[j].name)))
                   name_uniq = 0;
                if((i != j) && (!strcmp(students[i].surname, students[j].surname)))
                   surname_uniq = 0;
                if(!(surname_uniq || name_uniq))
                    break;
            }

            if(name_uniq || surname_uniq){ // *** first filter, conditions on the student
