...
int main(int argc, char *argv[]) {
    ...
    if (db){
        ...
        for (int i = 0; i < size; ++i){ // process all the student records in database
            ...
            char passed_philosophy = 0;
            for (int i = 0; i < s.load; ++i){ // process each course taken by the student
                if(!strcmp(s.courses[i], "Philosophy") && s.grades[i] > 4){
                    passed_philosophy = 1;
                    //break; have to pass all, if there are more than one philosophy course
                }
            }
            if(passed_philosophy){ // *** first filter, conditions on the student
